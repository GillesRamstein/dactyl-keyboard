...
def update(config, _c):
    """Update dictionary. Maybe check that there are no key collisions"""
    if False:
        assert set(config.keys()) & set(_c.keys()) == set(), "Duplicated Key Names!"
    config.update(_c)
...
config = {
    ...
    "save_dir": r"~/not-work-stuff/dactyl-keyboard/configs",
    "config_name": "iteration1",
    "show_caps": False, # "MX",  # False, "MX", "CHOC"
    "show_pcbs": False,  # False if show_caps=False; Easist place to initially inject geometry
}
...
_c = {
    ...
    "carbonfet_thumb_screw_xy_locations": [[-27, -57]],
...
_c = {
    ...
    "skeletal": False,
    ##############################
    "wall_z_offset": 5,  # 15 # length of the first downward_sloping part of the wall
    "wall_x_offset": 5,  # 5 # offset in the x and/or y direction for the first downward_sloping part of the wall (negative)
    "wall_y_offset": 5,  # 6 # offset in the x and/or y direction for the first downward_sloping part of the wall (negative)
    "left_wall_x_offset": 2, # 12  # specific values for the left side due to the minimal wall.
    "left_wall_z_offset": 1,  # 10? # specific values for the left side due to the minimal wall.
...
_c = {
    ...
    # 'UNDERCUT' = snap fit undercut.  May require CLIP_THICKNESS and possibly CLIP_UNDERCUT
    #      tweaking and/or filing to get proper snap.
    # 'NOTCH' = snap fit undercut only near switch clip.  May require CLIP_THICKNESS and
    #      possibly CLIP_UNDERCUT tweaking and/or filing to get proper snap.
...
# Screw mount points for base plate
_c = {
    "screws_offset": "INSIDE",  # 'OUTSIDE', 'INSIDE', 'ORIGINAL'
    "screw_insert_height": 5,
    'screw_insert_bottom_radius': 4.5 / 2,  #Designed for inserts
    'screw_insert_top_radius': 3 / 2,  #Designed for inserts
...
# Holes in Plate for screwed Single Key PCBs (https://github.com/ergohaven/su120-keyboard)
_c = {
    "plate_holes": True,
    "plate_holes_xy_offset": (0.0, 0.0),
    "plate_holes_width": 14.3,
    "plate_holes_height": 14.3,
    "plate_holes_diameter": 1.2,  # 1.6
    "plate_holes_depth": 4.5,  # 20.0
}
update(config, _c)


# EXPERIMENTAL - SHOW PCB FOR FIT CHECK
_c = {
...
if __name__ == "__main__":
    """Save config to <config_dir>/<config_name>.json"""
    p = Path(config['save_dir'], config['config_name']).with_suffix(".json").expanduser()
    with open(p, "w") as f:
        print(f"Saving config to {p}")
