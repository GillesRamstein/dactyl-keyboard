...
if len(sys.argv) <= 1:
    ...
    for opt, arg in opts:
        if opt not  in ("-c", "--config"):
            continue
        for p in (
            Path(r"..", "configs", arg + ".json"),
            Path("configs", arg + ".json"),
            Path(arg).expanduser(),
        ):
            if not p.is_file():
                continue
            print(f"Loading config from {str(p)}")
...
# Really rough setup.  Check for ENGINE, set if not present from configuration.
try:
    ENGINE = data["ENGINE"]
...
save_path = str(Path("~/not-work-stuff/dactyl-keyboard/things").expanduser())
parts_path = str(Path("~/not-work-stuff/dactyl-keyboard/src/parts").expanduser())
...
def carbonfet_thumbcaps():
    ...
    # CUSTOM CHANGE GILLES
    t15 = carbonfet_thumb_15x_layout(keycap(1))
    return t1.add(t15)
...
def model_side(side="right"):
    ...
    if debug_exports:
        ...
        export_file(
            shape=thumb_test,
            fname=path.join(r"..", "things", r"debug_thumb_test_{}_shape".format(side)),
        )

    thumb_section = union(
        [thumb_shape, thumb_connector_shape, thumb_wall_shape, thumb_connection_shape]
    )
    ...
    if debug_exports:
        export_file(
            shape=thumb_section,
            fname=path.join(r"..", "things", r"debug_thumb_test_5_shape".format(side)),
        )
